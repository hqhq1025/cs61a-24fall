def divide(quotients, divisors):
    ...
    """
    return {i : [d for d in divisors if d % i == 0] for i in quotients}
...
def distance(city_a, city_b):
    ...
    dis = sqrt(pow(dis_lat,2) + pow(dis_lon,2))
    return dis
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    def dis_to_position(lat,lon,city_a):
        dis_lat = (get_lat(city_a) - lat)
        dis_lon = (get_lon(city_a) - lon)
        dis = sqrt(pow(dis_lat,2) + pow(dis_lon,2))
        return dis
    dis_a = dis_to_position(lat,lon,city_a)
    dis_b = dis_to_position(lat,lon,city_b)
    if dis_a > dis_b:
