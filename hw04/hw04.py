def shuffle(s):
    ...
    list_a = s[0:len(s)//2]
    list_b = s[len(s)//2:]
    return [item for a, b in zip(list_a, list_b) for item in (a, b)]
...
def deep_map(f, s):
    ...
    """
    "*** YOUR CODE HERE ***"
    for item,index in zip(s,range(len(s))):
        if type(item) == list:
            deep_map(f,item)
        else:
            s[index] = f(s[index])   #易错点：这里不能直接更改item  而是应该更改s【i】
...
def balanced(m):
    ...
    if is_planet(m):
        ...
        left_mass = total_mass(end(left_arm))
        right_mass = total_mass(end(right_arm))
        if left_len*left_mass == right_len*right_mass:    #这里其实写复杂了
            return True and balanced(end(left_arm)) and balanced(end(right_arm))
        else:
            return False
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        return label(t)
    branch_sum = [max_path_sum(branch) for branch in branches(t)]
    return label(t) + max(branch_sum)
