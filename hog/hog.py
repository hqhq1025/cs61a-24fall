...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    dif = 3*abs(player_score % 10 - (opponent_score // 10)%10)
    if dif > 1:
        return dif
    else:
        return 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    check_num = strategy(0,0)
    for i in range(goal):
        for j in range(goal):
            if check_num != strategy(i,j):
                return False
    return True
    # END PROBLEM 7
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    if abs(score - sus_update(0,score,opponent_score,dice=six_sided)) >= threshold:    #注意这里调用sus中的 numroll 应该是0
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
