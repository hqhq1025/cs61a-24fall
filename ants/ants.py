...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b    #逻辑就是判断是否为容器——能否装下另一个——装进去   其中无非是判断self 还是 place.ant 哪个是容器
            assert (place.ant is None) or (self.can_contain(place.ant)) or (place.ant.can_contain(self)), 'Too many ants in {0}'.format(place)
            if place.ant.can_contain(self) and place.ant.is_container:
                place.ant.store_ant(self)
            elif self.can_contain(place.ant) and self.is_container:
                self.store_ant(place.ant)
                place.ant = self  #第一遍被卡住了：存进容器后  更新改位置的 ant 信息（设为容器）
            # END Problem 8b
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    upper_bound = float('inf')    #命名是有限制的  我debug 了半天
    lower_bound = -1
    def nearest_bee(self):
        ...
        place = self.place
        count = 0
        while place.entrance is not None:#卡了一会  大概逻辑就是判断前方的位置是否满足某种条件，然后选择 return 还是前进
            if count > self.upper_bound:
                return None
            if not place.is_hive :
                    if len(place.bees) > 0 and count >= self.lower_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"   #这里需要搞清楚造成伤害的逻辑顺序  我反正一遍没搞懂  需要认真读题
